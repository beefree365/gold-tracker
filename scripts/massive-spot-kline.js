...
const DEFAULT_OUTPUT_PATH = path.join(__dirname, '..', 'massive-spot-kline.png');
...
function formatTimestamp(value) {
    ...
    if (typeof value === 'number') {
        if (value > 1e12) {
            return new Date(value / 1e6).toISOString().replace('T', ' ').replace('Z', ' UTC');
        }
...
function normalizeBar(rawBar) {
    ...
    const open = Number(rawBar.open ?? rawBar.o ?? rawBar.Open ?? rawBar.open_price);
    const high = Number(rawBar.high ?? rawBar.h ?? rawBar.High ?? rawBar.high_price);
    const low = Number(rawBar.low ?? rawBar.l ?? rawBar.Low ?? rawBar.low_price);
    const close = Number(rawBar.close ?? rawBar.c ?? rawBar.Close ?? rawBar.close_price);
    const volume = Number(
        rawBar.volume ?? rawBar.v ?? rawBar.Volume ?? rawBar.volume_traded ?? rawBar.volume_24h ?? 0,
    );

    if (!Number.isFinite(open) || !Number.isFinite(high) || !Number.isFinite(low) || !Number.isFinite(close)) {
        throw new Error(`Invalid bar payload: ${JSON.stringify(rawBar)}`);
    }

    const normalizedTimestamp =
        typeof timestamp === 'number'
            ? new Date(timestamp > 1e12 ? timestamp / 1e6 : timestamp).toISOString()
            : new Date(String(timestamp)).toISOString();

    return {
        window_start: normalizedTimestamp,
        open,
        high,
        low,
        close,
...
function drawChart(bars, ticker) {
    const width = 1400;
    const height = 860;
    ...
    ctx.fillText(`Massive Spot 1-Minute K-Line (${bars.length} bars)`, padLeft, 30);

    const firstTs = bars[0].window_start;
    const lastTs = bars[bars.length - 1].window_start;
    ctx.font = '14px sans-serif';
    ctx.fillStyle = '#cbd5e1';
    ctx.fillText(`Ticker: ${ticker} | From ${formatTimestamp(firstTs)} to ${formatTimestamp(lastTs)}`, padLeft, 62);
...
async function fetchBars() {
    ...
    const ticker = getEnvOrDefault('MASSIVE_SPOT_TICKER', DEFAULT_TICKER);
    const multiplier = getEnvOrDefault('MASSIVE_SPOT_MULTIPLIER', DEFAULT_MULTIPLIER);
    const timespan = getEnvOrDefault('MASSIVE_SPOT_TIMESPAN', DEFAULT_TIMESPAN);
    const limit = getEnvOrDefault('MASSIVE_SPOT_LIMIT', DEFAULT_LIMIT);
    const defaultRange = getDefaultRange();
    const start = normalizeDateInput(process.env.MASSIVE_SPOT_START_DATE, new Date(defaultRange.start));
    const end = normalizeDateInput(process.env.MASSIVE_SPOT_END_DATE, new Date(defaultRange.end));

    const url = new URL('https://api.massive.com/v2/aggs/ticker');
    url.pathname += `/${encodeURIComponent(ticker)}/range/${encodeURIComponent(multiplier)}/${encodeURIComponent(timespan)}/${encodeURIComponent(start.slice(0, 10))}/${encodeURIComponent(end.slice(0, 10))}`;
    ...
    const response = await fetch(url.toString(), {
        headers: {
            'User-Agent': 'gold-tracker-massive-spot-kline/1.0',
        },
    });
    ...
    if (!payload || !Array.isArray(payload.results)) {
        throw new Error(`Massive response did not include results for ${ticker}`);
    }
    ...
    return payload.results.map(normalizeBar).sort((a, b) => new Date(a.window_start) - new Date(b.window_start));
}

async function main() {
    const ticker = getEnvOrDefault('MASSIVE_SPOT_TICKER', DEFAULT_TICKER);
    const outputPath = getEnvOrDefault('MASSIVE_SPOT_OUTPUT_PATH', DEFAULT_OUTPUT_PATH);

    const bars = await fetchBars();
    const canvas = drawChart(bars, ticker);
